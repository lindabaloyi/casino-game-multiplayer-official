...
const DraggableCard = ({
  ...
  const panResponder = PanResponder.create({
    ...
    onPanResponderRelease: (event, gestureState) => {
      ...
      // --- DEBUG LOGGING START ---
      console.log(`[DropAttempt] Card ${card.rank}${card.suit} dropped at (${dropPosition.x.toFixed(0)}, ${dropPosition.y.toFixed(0)})`);
      // --- DEBUG LOGGING END ---
      console.log(`[DropZones] Found ${global.dropZones?.length || 0} registered drop zones.`);
      ...
      if (global.dropZones) {
        ...
        if (bestZone) {
          ...
          // --- DEBUG LOGGING START ---
          console.log(`[DropSuccess] Best target found: Zone ${bestZone.stackId}. Attempting to drop.`);
          // --- DEBUG LOGGING END ---
          ...
          // --- DEBUG LOGGING START ---
          console.log('[DropFail] No suitable drop zone found in the area. This should result in a trail.');
          // --- DEBUG LOGGING END ---
        }
      }

      // Only animate the card back if the drop was not handled.
      // A handled drop means the card was successfully dropped on a target or trailed.
      if (!dropPosition.handled && source !== 'hand') {
        Animated.spring(pan, {
          toValue: { x: 0, y: 0 },
          useNativeDriver: false,
        }).start();
