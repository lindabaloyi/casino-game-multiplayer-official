 ...
 */
const getCardId = (card) => `${card.rank}-${card.suit}`;
 ...
 */
const isValidBuildType = (cards, targetValue) => {
 ...
 */
const findOpponentMatchingCards = (opponentCaptures, targetCard) => {
  const matchingCards = [];
 ...
 */
const countIdenticalCardsInHand = (playerHand, targetCard) => {
 ...
 */
const createCaptureStack = (capturingCard, capturedCards, opponentCard = null) => {
  if (opponentCard) {
