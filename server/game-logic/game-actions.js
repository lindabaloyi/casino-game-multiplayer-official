const { updateGameState, nextPlayer, logGameState } = require('./game-state.js');
const { rankValue, removeCardFromHand, removeCardsFromTable, sortCardsByRank, calculateCardSum, generateBuildId, findOpponentMatchingCards, createCaptureStack } = require('./card-operations.js');
const { canPartitionIntoSums } = require('./algorithms.js');
const { validateBuild, findPossibleBuildsFromStack } = require('./validation.js');

exports.handleTrail = (gameState, card) => {
  const { playerHands, tableCards, currentPlayer } = gameState;

  // Remove card from hand using the 0-based currentPlayer
  ...
  logGameState(`Player ${currentPlayer} trailed a ${cardRemoved.rank}`, nextPlayer(newState)); // Log with 1-based currentPlayer
  return nextPlayer(newState);
};

exports.handleBuild = (gameState, draggedItem, tableCardsInBuild, buildValue, biggerCard, smallerCard) => {
  ...
  const newBuild = {
    ...
    value: buildValue,
    owner: currentPlayer, // 0-based index
  ...
  logGameState(`Player ${currentPlayer} built a ${buildValue}`, nextPlayer(newState));
  return nextPlayer(newState);
};

exports.handleReinforceBuildWithStack = (gameState, stack, targetBuild) => {
  ...
  const newBuild = {
    ...
    owner: currentPlayer,      // Ownership is always transferred to the current player (0-based)
    isExtendable: false,       // Reinforced builds cannot be extended further
  };

  // Remove old build and temporary stack, then add the new build.
  ...
  logGameState(`Player ${currentPlayer} reinforced a build of ${targetBuild.value}`, nextPlayer(newState));
  return nextPlayer(newState);
};

exports.handleBaseBuild = (gameState, draggedItem, baseCard, otherCardsInBuild) => {
  const { playerHands, tableCards, playerCaptures, currentPlayer } = gameState;
  const { card: playerCard, source } = draggedItem;
  ...
  logGameState(`Player ${currentPlayer} created a base build with a ${playerCard.rank}`, nextPlayer(newState));
  return nextPlayer(newState);
};

exports.handleCreateBuildFromStack = (gameState, draggedItem, stack) => {
  ...
  const newBuild = {
    ...
    value: newBuildValue, // The value of the build
    owner: currentPlayer, // Ownership is always transferred to the current player (0-based)
    isExtendable: !isReinforce,
  };

  // 3. Update game state
  // Remove hand card using 0-based currentPlayer
  ...
  logGameState(`Player ${currentPlayer} created a build of ${newBuildValue} from a stack`, nextPlayer(newState));
  return nextPlayer(newState);
};

exports.handleAddToOpponentBuild = (gameState, draggedItem, buildToAddTo) => {
  const { playerHands, tableCards, playerCaptures, currentPlayer } = gameState;
  const { card: playerCard, source } = draggedItem;
  ...
  const newBuild = {
    ...
    cards: sortedCards,
    value: newBuildValue, // The value of the build
    owner: currentPlayer, // Ownership is transferred to the current player (0-based)
  ...
  if (source === 'table') {
    newTableCards = removeCardsFromTable(tableCards, [playerCard]); // This removes from table, no currentPlayer needed
  } else { // Default to hand for builds
    const removalResult = removeCardFromHand(playerHands, currentPlayer, playerCard);
    if (!removalResult) return gameState;
    newPlayerHands = removalResult.updatedHands;
  ...
  logGameState(`Player ${currentPlayer} extended opponent's build to ${newBuild.value}`, nextPlayer(newState));
  return nextPlayer(newState);
};

exports.handleAddToOwnBuild = (gameState, draggedItem, buildToAddTo) => {
  ...
  logGameState(`Player ${currentPlayer} added to their build, creating a new build of ${newBuildValue}`, nextPlayer(newState));
  return nextPlayer(newState);
};

exports.handleCapture = (gameState, draggedItem, selectedTableCards, opponentCard = null) => {
  const { playerHands, tableCards, playerCaptures, currentPlayer } = gameState;
  ...
  if (playerOwnBuild && source === 'hand' && !opponentCard && !isTargetingBuild) {
    ...
    if (canPartitionIntoSums(potentialStackCards, playerOwnBuild.value)) {
      ...
      logGameState(`Player ${currentPlayer} reinforced their build of ${playerOwnBuild.value}`, nextPlayer(newState));
      return nextPlayer(newState);
    }
  }
  ...
  console.log(`🎯 CAPTURE DEBUG: Player ${currentPlayer} capturing with ${selectedCard.rank}${selectedCard.suit} from ${source}`);
  console.log(`🎯 CAPTURE DEBUG: Player ${currentPlayer} capturing with ${selectedCard.rank}${selectedCard.suit} from ${source}`);
  ...
  const captureDescription = opponentCard
    ? `Player ${currentPlayer} captured with a ${selectedCard.rank} (using opponent's ${opponentCard.rank})`
    : `Player ${currentPlayer} captured with a ${selectedCard.rank}`;

  logGameState(captureDescription, nextPlayer(newState, currentPlayer));
  return nextPlayer(newState);
};
 ...
 */
exports.startNextRound = (gameState) => {
 ...
 */
exports.calculateScores = (playerCaptures) => {
  const details = [
 ...
 */
exports.handleCreateStagingStack = (gameState, handCard, tableCard) => {
  const { playerHands, tableCards, currentPlayer } = gameState;
  ...
  const removalResult = removeCardFromHand(playerHands, currentPlayer, handCard);
  if (!removalResult) {
    console.error(`Error: Expected hand card ${handCard.rank}${handCard.suit} not found in player ${currentPlayer}'s hand during staging stack creation.`);
    return gameState;
  }
  const newPlayerHands = removalResult.updatedHands;
  const actualCardUsed = removalResult.cardRemoved;
 ...
 * @returns {object} The updated game state.
 */
exports.handleAddToStagingStack = (gameState, handCard, targetStack) => {
  const { playerHands, tableCards, currentPlayer } = gameState;
 ...
 * @returns {object} The updated game state.
 */
exports.handleDisbandStagingStack = (gameState, stackToDisband) => {
  let { playerHands, tableCards, playerCaptures, currentPlayer } = gameState;
  ...
  const newState = updateGameState(gameState, { playerHands: newPlayerHands, tableCards: newTableCards, playerCaptures: playerCaptures }); // Pass playerCaptures
  logGameState(`Player ${currentPlayer}'s temporary stack was invalid and disbanded.`, nextPlayer(newState));
  return nextPlayer(newState);
};
 ...
 * @returns {object} The updated game state.
 */
exports.handleCancelStagingStack = (gameState, stackToCancel) => {
  let { playerHands, tableCards, playerCaptures, currentPlayer } = gameState;
  ...
  // playerCaptures is already updated by reference in the previous step
 ...
 * @returns {object} The updated game state.
 */
exports.handleMergeIntoOwnBuild = (gameState, stack, targetBuild) => {
 ...
 */
exports.handleFinalizeStagingStack = (gameState, stack) => {
  const { playerHands, tableCards, currentPlayer } = gameState;
  const playerHand = playerHands[currentPlayer];
  ...
  logGameState(`Player ${currentPlayer} finalized a build of ${newBuild.value}`, nextPlayer(newState));
  return nextPlayer(newState);
};
 ...
 * @returns {object} The updated game state.
 */
exports.handleCreateBuildWithValue = (gameState, stack, buildValue) => {
  ...
  const newBuild = {
    ...
    value: buildValue, // The value of the build
    owner: currentPlayer, // Ownership is always transferred to the current player (0-based)
    isExtendable: true, // Builds are extendable by default
  };
  ...
  logGameState(`Player ${currentPlayer} created a build of ${buildValue}`, nextPlayer(newState));
  return nextPlayer(newState);
};
 ...
 * @returns {object} The updated game state.
 */
exports.handleStageSingleCardFromHand = (gameState, card) => {
  const { playerHands, tableCards, currentPlayer } = gameState;
 ...
 * @returns {object} The updated game state.
 */
exports.handleFinalizeTrail = (gameState, stack) => {
  const { tableCards, currentPlayer } = gameState;
  ...
  const newTableCards = tableCards.filter(s => s.stackId !== stack.stackId);
  newTableCards.push(cardToTrail); // Add the card to the table

  const newState = updateGameState(gameState, { tableCards: newTableCards });
  logGameState(`Player ${currentPlayer} trailed a ${cardToTrail.rank}`, nextPlayer(newState));
  return nextPlayer(newState);
};
 ...
 * @returns {object} The updated game state.
 */
const handleReinforceOpponentBuildWithStack = (gameState, stack, opponentBuild) => {
  const { tableCards } = gameState;
...
module.exports = {
  handleTrail: exports.handleTrail,
  handleBuild: exports.handleBuild,
  handleReinforceBuildWithStack: exports.handleReinforceBuildWithStack,
  handleBaseBuild: exports.handleBaseBuild,
  handleCreateBuildFromStack: exports.handleCreateBuildFromStack,
  handleAddToOpponentBuild: exports.handleAddToOpponentBuild,
  handleAddToOwnBuild: exports.handleAddToOwnBuild,
  handleCapture: exports.handleCapture,
  startNextRound: exports.startNextRound,
  handleSweep: exports.handleSweep,
  calculateScores: exports.calculateScores,
  endGame: exports.endGame,
  handleCreateStagingStack: exports.handleCreateStagingStack,
  handleAddToStagingStack: exports.handleAddToStagingStack,
  handleDisbandStagingStack: exports.handleDisbandStagingStack,
  handleCancelStagingStack: exports.handleCancelStagingStack,
  handleMergeIntoOwnBuild: exports.handleMergeIntoOwnBuild,
  handleStageOpponentCard: exports.handleStageOpponentCard,
  handleExtendToMerge: exports.handleExtendToMerge,
  handleFinalizeStagingStack: exports.handleFinalizeStagingStack,
  handleCreateBuildWithValue: exports.handleCreateBuildWithValue,
  handleStageSingleCardFromHand: exports.handleStageSingleCardFromHand,
  handleFinalizeTrail: exports.handleFinalizeTrail,
  handleReinforceOpponentBuildWithStack: exports.handleReinforceOpponentBuildWithStack,
};
 ...
 * @returns {object} The updated game state.
 */
exports.handleStageOpponentCard = (gameState, opponentCard) => {
 ...
 * @returns {object} The updated game state.
 */
exports.handleExtendToMerge = (gameState, handCard, opponentBuild, ownBuild) => {
  const { playerHands, tableCards, currentPlayer } = gameState;
 ...
 * @returns {object} The updated game state.
 */
exports.handleReinforceOpponentBuildWithStack = (gameState, stack, opponentBuild) => {
  const { tableCards } = gameState;
